...
function addBareDiaToCart(goodsId, parentId)
{
  ...
  var quick		   = 0;
  var sp_attr="";
  sp_attr+="钻重："+formBuy.elements['my_weight'].value;
  sp_attr+="<br/>证书："+formBuy.elements['my_Lab'].value;
  sp_attr+="<br/>证书编号："+formBuy.elements['my_LabNumber'].value;
  sp_attr+="<br/>切工："+formBuy.elements['my_Cut'].value;
  sp_attr+="<br/>净度："+formBuy.elements['my_Clarity'].value; 

  if(formBuy.elements['gxkz']){
	  var  kezhi=formBuy.elements['gxkz'].value; 
	  if(kezhi=="中文 四个字以内 英文 九个字以内")
		 kezhi="";
	  sp_attr+="<br/>个性刻字："+kezhi;
  }
  // 检查是否有商品规格 
  if (formBuy)
  {
...
function addBareDiaToCart2(goodsId, parentId)
{
  ...
  var quick		   = 0;
  var sp_attr="";
  sp_attr+="钻重："+my_weight;
  sp_attr+="<br/>证书："+my_Lab;
  sp_attr+="<br/>证书编号："+my_LabNumber;
  sp_attr+="<br/>切工："+my_Cut;
  sp_attr+="<br/>净度："+my_Clarity;  
  // 检查是否有商品规格 
  if (formBuy)
  {
...
function addBareDiaToCart3(goodsId, parentId)
{
  ...
  sp_attr+="<br/>切工："+my_Cut;
  sp_attr+="<br/>净度："+my_Clarity;  */
  sp_attr+="钻重："+$("#weight").html();
  sp_attr+="<br/>证书："+$("#lab").html();
  sp_attr+="<br/>证书编号："+$("#labNumber").html();
  sp_attr+="<br/>切工："+$("#cut").html();
  sp_attr+="<br/>净度："+$("#clarity").html();
  // 检查是否有商品规格 
  if (formBuy)
  {
...
function buyBareDiamond(refno){
 ...
 document.getElementById("buy_div").innerHTML="<img src='/luozuan/images/carting.gif'>&nbsp;正在放入购物车";
...
function getZtGoodsInfo(id){
  document.getElementById("dr_goods_right").innerHTML="正在读取信息...";
...
function luozuan_dingzhi(){
  ...
  if(lz_id==""){
    ...
    if(confirm("您确定要定制此项组合么？")){
	  document.getElementById("dingzhi_img").innerHTML+="<img src='/luozuan/images/carting.gif'>&nbsp;正在为您定制,请稍候..."
...
function add_dingzhi_tocart(goodsId)
{
  ...
  var quick		   = 0;
  var sp_attr="";
  sp_attr+="钻重："+document.getElementById("lz_weight").value;
  sp_attr+="<br/>证书："+document.getElementById("lz_lab").value;
  sp_attr+="<br/>证书编号："+document.getElementById("lz_labnumber").value;
  sp_attr+="<br/>切工："+document.getElementById("lz_cut").value;;
  sp_attr+="<br/>净度："+document.getElementById("lz_clarity").value;;  
  // 检查是否有商品规格 
  if (formBuy)
  {
